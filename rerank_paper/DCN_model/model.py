...
class DNN:
    ...
    def _cross_network(self):
        # 3. cross network
        input_size = FEATURE_CATE_NUM * CATE_FEATURE_EMBEDDINGS_SHAPE[1]
    ...
    def _build_model(self, features, labels, mode, params):
        ...
        with tf.name_scope('dnn_model'):
            self.embeddings = self.input_embedding
            self._cross_network()
            fc_out = tf.concat([self.input_embedding,self.y_cross], axis=1) # Batch_size * POI_NUM * FEAT_NUM
            ...
            fc_out = tf.concat([fc_out, self.feat_predict], axis=1)
    ...
    def _create_weights(self):
        with tf.name_scope('feature_emb_weights'):
            ...
            # 3. cross layers
            self.cross_deep = CROSS_LAYERS_NUM
            self.cross_wides = CROSS_LAYERS
            num_layer = self.cross_deep
            input_size = FEATURE_CATE_NUM * CATE_FEATURE_EMBEDDINGS_SHAPE[1]
            weights = {}
            glorot = np.sqrt(2.0 / (input_size + self.cross_wides[0]))
            weights["cross_layer_0"] = tf.Variable(
                np.random.normal(loc=0, scale=glorot, size=(self.cross_wides[0], 1)),
                dtype=np.float32)
            ...
            for i in range(1, num_layer):
                ...
                weights["cross_layer_%d" % i] = tf.Variable(np.random.normal(loc=0.0, scale=glorot,
                                                                             size=(self.cross_wides[i], 1)),
                                                            dtype=np.float32)
                weights["cross_bias_%d" % i] = tf.Variable(
                    np.random.normal(loc=0.0, scale=glorot, size=(1, 1, self.cross_wides[i])),
                    dtype=np.float32)
