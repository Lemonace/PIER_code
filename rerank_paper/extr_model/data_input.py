...
def feature_product(parsed_features):
    ...
    cur_page_features = tf.reshape(tf.gather(feature_buffer, list(range(current_page_start, current_page_end)), axis=1),
                                   [-1, POI_NUM, FEATURE_NUM])
    pre_page_features = tf.reshape(tf.gather(feature_buffer, list(range(pre_page_start, pre_page_end)), axis=1),
                                   [-1, PAGE_NUM, POI_NUM, FEATURE_NUM_FOR_PAGE])
    ...
    _shape = tf.shape(adid_fea)
    fea_mask = tf.ones_like([_shape[0], 1]) * FEATURE_NATURE_POI
